...
- **Hook forensic log.** Both lifecycle hooks append one JSONL line per fire to `~/.core/hooks-log.jsonl` (override `CORE_HOOKS_LOG_FILE`) recording the decision they made — `inject` / `spawn` / `skip` with the reason. `cat` it to confirm the hooks are running and what they're doing.
