...
### Added
- **Opt-in rich-context capture (off by default).** A new local capture stream saves the literal query text and the delivered context-pack for a *bad* retrieval outcome (a miss, a zero-hit, or a corrective retry) so a developer can debug WHY a retrieval failed with full context — the closed-schema telemetry records that it happened but not why. It is OFF by default and activates only when a project's `workspace.json` carries `rich_context_capture: true`. When on, `/metrics` shows one plain-language line stating it is active, what it saves, and how to turn it off (nothing renders when off). Rows are schema-versioned JSONL written under a file lock to `<metrics-storage-base>/rich-context/<date>.jsonl`; they are retained 30 days by default (retention runs inside `maintenance-run.mjs` with dry-run and deletion proof) and can be purged on an explicit ask (`maintenance-run.mjs --purge-rich-context`). The stream is physically separate from the aggregate metrics and is NEVER read by the `/metrics-package` exporter — a permanent canary tripwire test asserts no rich-context content can reach an exported package. The first-run metrics disclosure now also notes this optional stream exists, is off by default, and what turning it on captures.
