...
Either path installs the skill, four sub-skills, and two hooks. Your `~/.claude/settings.json` is not modified. See [INSTALL.md](INSTALL.md) for local-file installs, one-session-only loading via `--plugin-dir`, the `--append-system-prompt` template, and migration notes if you had an older clone.

Four sub-skills ship bundled and are invocable as top-level slash commands: **`/orient`** (thread resumption), **`/finalize`** (session close with handoff + hygiene), **`/vibecheck`** (session vibe captured as ASCII, logged to `~/.core/vibes/`), and **`/file-reorganize`** (clean version-qualifier chaos and content-staleness from any directory). Two hooks register automatically: a skill-edit PWD guard and a per-turn voice reminder.
