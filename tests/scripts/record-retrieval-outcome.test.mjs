...
test('writes one evidence-qualified outcome for an existing retrieval', () => {
  ...
  try {
    const result = recordRetrievalOutcome(project, {
      retrieval_id: 'r-1', usefulness_outcome: 'partial', evidence_kind: 'user-confirmed',
    }, { today: '2026-07-17', now: '2026-07-17T03:00:00Z' });
    assert.equal(result.written, true);
    assert.deepEqual(result.record, {
      kind: 'retrieval-outcome', retrieval_id: 'r-1', usefulness_outcome: 'partial', evidence_kind: 'user-confirmed',
    });
...
test('rejects unknown retrieval ids without writing', () => {
  ...
  try {
    assert.throws(() => recordRetrievalOutcome(project, {
      retrieval_id: 'missing', usefulness_outcome: 'useful', evidence_kind: 'agent-judgment',
...
test('rejects duplicate or relabeled outcomes', () => {
  ...
  try {
    recordRetrievalOutcome(project, {
      retrieval_id: 'r-1', usefulness_outcome: 'useful', evidence_kind: 'answer-citation',
    }, { today: '2026-07-17' });
    assert.throws(() => recordRetrievalOutcome(project, {
      retrieval_id: 'r-1', usefulness_outcome: 'miss', evidence_kind: 'user-confirmed',
...
test('rejects invalid closed-vocabulary values', () => {
  ...
  try {
    assert.throws(() => recordRetrievalOutcome(project, {
      retrieval_id: 'r-1', usefulness_outcome: 'great', evidence_kind: 'user-confirmed',
...
test('fails closed while another outcome writer owns the scan-and-append lock', () => {
  ...
  try {
    ...
    assert.throws(() => recordRetrievalOutcome(project, {
      retrieval_id: 'r-1', usefulness_outcome: 'useful', evidence_kind: 'user-confirmed',
    }), /writer is locked/i);
    const rows = readFileSync(join(project, '_sessions', '2026-07-17', 'retrieval-log.jsonl'), 'utf8').trim().split('\n');
    assert.equal(rows.length, 1);
  } finally { rmSync(project, { recursive: true, force: true }); }
