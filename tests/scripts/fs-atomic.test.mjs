...
test('SYN-012: Windows EPERM is retried and succeeds on a later attempt', () => {
  let calls = 0;
...
test('SYN-012: Windows EPERM exhausts the retry budget and throws', () => {
  let calls = 0;
...
test('SYN-012: POSIX never retries — one attempt, immediate throw', () => {
  let calls = 0;
  const dead = () => { calls++; throw eperm(); };
...
test('SYN-012: non-transient Windows errors throw immediately (no pointless retry)', () => {
  let calls = 0;
...
test('SYN-012: the folder-rename protocol carries the cloud-sync caveat (doc guard, PROTO-009)', () => {
...
test('SYN-012: scripts/README.md documents the cloud-sync write caveats', () => {
