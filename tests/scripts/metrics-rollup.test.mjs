...
test('buildRollup computes the headline rec-fail-tier-0 rate for today', () => {
  withClassified({
    ...
    const r = buildRollup({ project, today: '2026-06-02', home, workspaceId: WID });
...
test('writeRollup writes the daily md + the orient-signal.txt that readOrientSignal reads', () => {
  withClassified({
    ...
    const r = writeRollup(buildRollup({ project, today: '2026-06-02', home, workspaceId: WID }));
    ...
    const signal = readOrientSignal(project, { home, workspaceId: WID });
...
test('readOrientSignal returns null when no signal has been written', () => {
  withClassified({}, ({ home, project }) => {
    assert.equal(readOrientSignal(project, { home, workspaceId: WID }), null);
  });
});
