...
A catalog of dispositions for multi-agent work. Use them as composable shapes the DM reaches for as the task warrants — not as menu items to look up by problem type. The selection call is judgment; the catalog gives the agent a vocabulary for that judgment.
...
These are tendencies, not assignments. The same task can warrant different dispositions at different phases, and a single phase can blend dispositions. Composition is the primary practice (per DC-76); the catalog is starting material, not a menu the DM reads off.
