...
## Graduation — the high-value reasoning step
...
- Pattern recognition: multiple observations connect into a substantive implication.
- Continuous self-evaluation flags "this keeps mattering."
- Explicit user cue ("this matters," "remember this").
...
When in doubt, write the unit. A slightly-too-eager unit is cheap. A missed critical fact isn't.

### Hard calls go to multi-agent

If graduation reasoning is genuinely shaky — should this be one unit or three, what's the right edge structure, which existing unit does this supersede — invoke `protocols/analysis.md`. Multi-agent is worth the cost when you're classifying things or making structural calls.
...
3. **Re-tune priority weights** — adjust `w_R`, `w_F`, `w_S`, or `w_A` in `_memories/_lib/priority.py`. Cheap; document the change in the hygiene retrospective (`~/.core/hygiene-cycles/<YYYY-MM-DD>.md`).
...
2. `_memories/INDEX-decisions.md` is auto-generated by `<project>/.claude/scripts/generate-decisions-index.py` — walks `_memories/dc-*.md`, parses frontmatter, sorts chronologically.
3. DECISIONS.md is archived to `<project>/archive/<YYYY-MM-DD>/DECISIONS-pre-graduation.md`.
...
- Not a replacement for the user's editorial judgment. The agent surfaces candidates; the user accepts, rejects, or refines.
- Not silent. Every operation is logged and narrated.
- Not a substitute for graduation. Archive is "less reachable," not "no longer true." Retire is "no longer current truth, keep the trace."
- Not a one-time pass. The mechanism is continuous; the `/finalize` pass is just the heaviest pass.
