...
3. **Re-tune priority weights** — adjust `w_R`, `w_F`, `w_S`, or `w_A` in the plugin's `scripts/priority.py`. Weights live in the plugin per DC-77 so the tuning propagates to every project via the next plugin update — not per-project drift. Document the change in the hygiene retrospective (`~/.core/hygiene-cycles/<YYYY-MM-DD>.md`).
...
2. `_memories/INDEX-decisions.md` is auto-generated by `~/.claude/skills/core/scripts/generate-decisions-index.py` (ships with the plugin per DC-77) — walks `_memories/dc-*.md`, parses frontmatter, sorts chronologically. Invoke from the project root or pass an explicit memories dir; the script overwrites `INDEX-decisions.md` in place.
