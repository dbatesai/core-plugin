...
Echo the script's stdout verbatim into the readiness summary as a quoted line — exact characters, no paraphrase, no rewording. If it printed `forked probe-codex-3 -> codex-fork-test; registered at /Users/dbates/.core/workspaces/codex-fork-test/`, the readiness must contain that exact string. After the verbatim echo, you may add a plain-voice gloss in a separate sentence (e.g., *"That means this `workspace.json` was copied from `probe-codex-3`; we're treating it as a new workspace."*) — but the gloss is supplemental, never a replacement. If the script printed `(no fork needed)`, no narration required.
