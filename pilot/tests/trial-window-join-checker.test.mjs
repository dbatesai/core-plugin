...
// Hale falsifier 1 (hale--de3066d-window-design-pass-five-fail-open-hold):
...
// Any contamination in the window is now a spoil before arm is even
// checked.
test('Hale falsifier 1: a second retrieval for a DIFFERENT arm inside the window is still contamination, not invisible', () => {
  const root = store();
  try {
    const retrievalBefore = captureCursor(root, DATE, 'retrieval-log.jsonl');
    const outcomeBefore = captureCursor(root, DATE, 'outcome-log.jsonl');
    ...
    plantRetrieval(root, { retrievalId: 'r-1', arm: 'memory-on' });
    plantRetrieval(root, { retrievalId: 'r-2', arm: 'memory-off' }); // different arm — old code ignored this
    plantImmediateOutcome(root, { retrievalId: 'r-1' });
...
// Hale falsifier 2: a second outcome for a DIFFERENT retrieval_id inside the
// outcome window used to be invisible (the old code filtered to the target
// retrieval_id before counting). Any contamination is now a spoil first.
test('Hale falsifier 2: a second outcome for a DIFFERENT retrieval_id inside the window is still contamination, not invisible', () => {
  const root = store();
  try {
    ...
    plantRetrieval(root, { retrievalId: 'r-unrelated', arm: 'memory-off' });
...
// Hale falsifier 3: every expected-identity argument used to be optional;
// omitting all four still returned ok:true. They are now required, same
// tier as expectedArm.
test('Hale falsifier 3: omitting expected identity throws instead of silently skipping the check', () => {
  const root = store();
  try {
    const retrievalBefore = captureCursor(root, DATE, 'retrieval-log.jsonl');
    const outcomeBefore = captureCursor(root, DATE, 'outcome-log.jsonl');
...
// Hale falsifier 4: a malformed/non-object JSON line inside the window used
// to be silently dropped by the parser instead of spoiling the check.
test('Hale falsifier 4: a malformed JSON line inside the window is a spoil reason, not a silent drop', () => {
  const root = store();
  try {
    const retrievalBefore = captureCursor(root, DATE, 'retrieval-log.jsonl');
    const outcomeBefore = captureCursor(root, DATE, 'outcome-log.jsonl');
...
// Hale falsifier 5: a negative `before` cursor used to be silently clamped
// to zero, silently widening the caller's own declared window.
test('Hale falsifier 5: a negative cursor throws instead of being silently clamped to zero', () => {
  const root = store();
  try {
    plantRetrieval(root, { retrievalId: 'r-1', arm: 'memory-on' });
    plantImmediateOutcome(root, { retrievalId: 'r-1' });
...
test('checkTrialWindow requires explicit cursors — refuses to fall back to a global scan', () => {
  const root = store();
  try {
    assert.throws(() => checkTrialWindow(root, { date: DATE, expectedArm: 'memory-on', ...EXPECTED_IDENTITY }), /retrievalWindow and outcomeWindow/);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
