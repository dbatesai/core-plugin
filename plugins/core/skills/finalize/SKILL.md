...
3. **Run mechanical maintenance.** Run the consolidated mechanical pass — it regenerates both indexes + the summary index, cleans cloud-sync ghost duplicates, and checks the PROJECT.md cap, all signature-gated (cheap on an unchanged store) and recorded in the cadence ledger:
   ...
   Ships with the plugin by design. Surface its one-line narration to the user (it's never silent — visible-continuous-curation). Heavier hygiene ops (demote-moves, compaction, validation) stay in their own steps below; this consolidates the cheap deterministic regen/cleanup that used to be separate `generate-*-index.mjs` calls.
...
Scale the narrative to the session: a trivial session gets honest one-liners under each header, not padding. **Compose from this session's transcript, PROJECT.md, and the units written this session — never from prior summaries.** A prior summary is last session's narrative, not current state: the 2026-07-01 headless close copied a stale "WIP" claim out of an old summary and recorded it as current truth (`obs-20260702-headless-task-a-anchored-stale-summary`). That anchoring failure is why this rule exists.
