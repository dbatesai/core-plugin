...
import { readFileSync, readdirSync, appendFileSync, realpathSync, existsSync } from 'node:fs';
...
// ---------- Self-maintaining top-N block (Crest/bblens ask, 2026-07-21) ----------
//
// MEMORY.md's "top project units" pointer list has historically been hand-curated
// during /finalize — a hygiene burden that drifts, and the exact gap Crest named
// from operating bblens at 158-unit scale. This regenerates a marker-delimited
// block in place from the same ranking the CLI prints, so the index stays current
// without a human (or an LLM narrating "I refreshed it") re-deriving the list.

export const PRIORITY_TOP_BEGIN = '<!-- PRIORITY-TOP:BEGIN -->';
export const PRIORITY_TOP_END = '<!-- PRIORITY-TOP:END -->';

/**
 * One-line pointer per unit, relative to `baseDir` when given (so the generated
 * markdown link resolves the same way hand-authored MEMORY.md entries do).
 */
export function renderPriorityTopBlock(ranked, topN, { baseDir = null, now = null } = {}) {
  const top = ranked.slice(0, topN);
  const lines = top.map(([, u]) => {
    const p = baseDir ? relative(baseDir, String(u.path)) : String(u.path);
    const topics = (u.fm.topics || []).join(', ');
    const type = u.fm.type || 'unit';
    const tail = topics ? `${type}; topics: ${topics}` : type;
    return `- [${u.id}](${p}) — ${tail}`;
  });
  const stamp = now || new Date().toISOString();
  const body = lines.length ? lines.join('\n') : '*(no active units ranked)*';
  return `${PRIORITY_TOP_BEGIN}\n## Top project units (auto-regenerated by priority.mjs --inject-top, ${stamp.slice(0, 10)})\n\n${body}\n${PRIORITY_TOP_END}`;
}

function findPriorityTopBlock(text) {
  const start = text.indexOf(PRIORITY_TOP_BEGIN);
  if (start === -1) return null;
  const endTagIdx = text.indexOf(PRIORITY_TOP_END, start);
  if (endTagIdx === -1) return null;
  return { start, end: endTagIdx + PRIORITY_TOP_END.length };
}

/**
 * Regenerate the top-N block in `filePath` in place (e.g. a project's MEMORY.md).
 * Replaces an existing marker-delimited block; appends a new one (with markers)
 * if none exists yet, so first-run adoption doesn't require hand-editing the file
 * first. Missing file → treated as empty (block becomes the whole file).
 * @returns {{ written: boolean, path: string }}
 */
export function injectPriorityTop(filePath, ranked, topN, { now = null, baseDir = null } = {}) {
  const abs = resolve(filePath);
  const original = existsSync(abs) ? readFileSync(abs, 'utf8') : '';
  const block = renderPriorityTopBlock(ranked, topN, { baseDir: baseDir || resolve(abs, '..'), now });
  const existing = findPriorityTopBlock(original);
  let updated;
  if (existing) {
  ...
  if (updated !== original) atomicWriteFileSync(abs, updated);
  return { written: updated !== original, path: abs };
}
...
export function main(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--inject-top') { injectTopPath = argv[++i]; }
  ...
  if (injectTopPath) {
    const result = injectPriorityTop(injectTopPath, ranked, topN, { now: today.toISOString() });
    console.log(result.written
      ? `Regenerated top-${topN} block in ${result.path}`
      : `${result.path} already current — no change`);
    return 0;
  }
