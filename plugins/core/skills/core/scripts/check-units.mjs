...
// Generated render artifacts are not memory units, but prose inside a real unit can
// mention "Render-only artifact". Keep this producer/path-scoped so body text alone
// cannot hide a unit from schema or integrity validation.
const RENDER_ONLY_ARTIFACTS = new Map([
  ['capability-drift-log.md', 'Regenerated by analyze-capability-drift.mjs'],
]);

function fileIsRenderOnlyArtifact(p) {
  const marker = RENDER_ONLY_ARTIFACTS.get(basename(p));
  if (!marker) return false;
  try {
    const text = readFileSync(p, 'utf8');
    if (text.startsWith('---\n')) return false;
    return text.includes('Render-only artifact') && text.includes(marker);
  } catch { return false; }
}
...
export function iterActiveUnits(memoriesDir) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    try {
      if (fileIsRenderOnlyArtifact(join(memoriesDir, name))) continue;
...
export function iterAllUnitFiles(memoriesDir) {
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (fileIsRenderOnlyArtifact(join(dir, entry.name))) continue;
