...
| `agent_notes` | string | yes | Free-form agent operational notes — cross-session observations about how the project is being worked on. Project facts (decisions, risks, moves, people) live in `<project>/PROJECT.md`, not here. *Renamed from `dm_notes` 2026-07-16; manifests written by older versions still carry `dm_notes` — read it as the same field, and migrate the key when you next rewrite the manifest.* |
