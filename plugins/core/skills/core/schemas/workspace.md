...
| `rich_context_capture` | boolean | no | **Retired (v3.14.0).** The old opt-in rich-context stream was superseded by the every-turn turn-capture evidence layer, whose opt-OUT flag `turn_capture: false` lives in the **project-root** `workspace.json` pointer (an opt-out travelling with a copied project is privacy-safe — the inverse of the sensitivity reasoning that kept this opt-in machine-local). A leftover `rich_context_capture` value here is ignored; the maintenance pass removes any leftover `rich-context/` stream directory. See `protocols/data-storage.md` §"Two capture streams". |
