...
| `agent_notes` | string | yes | Free-form agent operational notes — cross-session observations about how the project is being worked on. Project facts (decisions, risks, moves, people) live in `<project>/PROJECT.md`, not here. *Renamed from `dm_notes` 2026-07-16 (DC-119); manifests written by older versions still carry `dm_notes` — read it as the same field, and migrate the key when you next rewrite the manifest.* |
...
| `metrics_disclosure_shown` | boolean | no | Written by `scripts/metrics-disclosure.mjs` the first time the workspace's new-workspace scaffold runs. `true` means the first-run metrics-capture notice (dc-107) has already appeared in a readiness summary for this workspace — never show it again. Absent/`false` = not yet shown. |
| `rich_context_capture` | boolean | no | The per-user opt-in for the sensitive rich-context capture stream. `true` activates local capture of a bounded head of query + delivered-context text on synchronous no-hit retrievals (see `protocols/data-storage.md` §"Two capture streams"). **Deliberately lives ONLY in this machine-local per-user manifest, never the project-root pointer:** the pointer travels with a copied or shared project, and a sensitive-capture choice must not ride along to a teammate. Read by `scripts/rich-context-capture.mjs`'s `richContextCaptureEnabled()`. Absent/`false` = OFF (the shipped default). |
