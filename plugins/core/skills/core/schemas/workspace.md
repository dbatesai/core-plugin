...
| `contract_path` | string | no | (v3.0) Path to the project's `CONTRACT.md` — the canonical source the per-harness `CLAUDE.md`/`AGENTS.md`/`GEMINI.md` are generated from. **Omit when the contract is at the default `<project>/CONTRACT.md`** (the generators resolve that automatically); set it only for nonstandard layouts (e.g. `docs/CONTRACT.md`). Absent field = the project hasn't adopted the contract→generator system, and the contract-drift release gate is skipped. |
