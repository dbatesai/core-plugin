...
| `dm_notes` | string | yes | Free-form agent operational notes — cross-session observations about how the project is being worked on. Project facts (decisions, risks, moves, people) live in `<project>/PROJECT.md`, not here. |
...
- **`dm_notes` is free-form.** Agent scratch space for operational observations. No structure imposed. Use it for cross-session hunches, user interaction patterns, agent-side process observations — never for facts that belong in `PROJECT.md`.
