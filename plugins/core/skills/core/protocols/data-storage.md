...
- **PROJECT.md** — `hot-section.mjs apply` stamps `last_written_by: hot-section` (Hale's finding, 2026-07-21). Use `classifyProjectMdChange(cachedStamp, currentText)` from `hot-section.mjs`: it hashes only the content outside the marker-delimited hot block, which `hot-section.mjs` never touches by construction. `'hot-block-only'` is CORE's synthesis: refresh the entry, don't propagate or fire anti-resurrection. `'outside-changed'` or `'no-baseline'` (a pre-fix stamp with no `outside_hash`) must be treated as a genuine user edit.
