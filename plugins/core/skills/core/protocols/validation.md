...
**Sharing measurement evidence across a project boundary** uses two surfaces, never one. The harness report and tier-sweep output are the LOCAL evidence bundle — gold rows, raw ranks, per-query bands, store paths — and stay in the environment that produced them. What travels is the aggregate receipt built by `aggregate-receipt.mjs` (`node aggregate-receipt.mjs <report.json> [--sweep <sweep.json>] [--out <receipt.json>]`): whitelist-constructed rates, counts, latencies, and hashes, then refusal-scanned against every unit id, query text, and path the local report saw — a hit refuses the whole export with exit 2. If the receipt won't build, the evidence wasn't shareable; fix the input, never hand-edit the receipt.

**Identifying a frozen release artifact** uses `artifact-identity.mjs`, never tar-byte hashes (tar embeds invocation-time metadata — same content, different bytes every run). `node artifact-identity.mjs <repo> <ref> [--subdir plugins/core]` prints the Git tree object id (one-command verification for anyone with the repository) and a content-manifest sha256 (sorted `relpath:sha256(bytes)` over the subtree) that any export of the tree reproduces without git — `node artifact-identity.mjs --dir <extracted-tree>` computes the same hash from an extracted archive or a packaged install. An evidence packet publishes both identities plus these exact commands; a receipt's `built_artifact_sha256` slot carries the content-manifest hash.
