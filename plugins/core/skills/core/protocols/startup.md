...
This is a backstop, not a replacement for the `/process-memory` wiring, which runs decoration and index regeneration as part of its pass. The two layers are deliberately redundant on a healthy store: on any ordinary session, close-time maintenance already left the store current, so this step is a fast no-op. Its value is entirely in the failure mode close-time maintenance can't self-detect — an op quietly missing from the tracked list, or bookkeeping that's wrong about what it covers — which only shows up if something runs the real thing directly, on a fixed schedule, without asking the ledger for permission first.
