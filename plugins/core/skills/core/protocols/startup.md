...
**Lifecycle preflight — record the session-start inventory, then classify (Hale's 2026-07-22 boundary fix).** Before edit-detection reads anything and before any writer runs, snapshot which user-sensitive files exist right now and get one machine-readable read of the store's state. The `--record-session-start` snapshot is what lets a no-cache-baseline file be told apart later: a file present now with no stamp is a genuine pre-existing file (a user could have created it between sessions — treat conservatively); a file that only appears after this point is one CORE created this session (the normal graduation/first-write path). Run it ONCE, here, before the decoration backstop below (guarded like every script call):
...
Its per-file `classification` feeds the edit-detection below and the decoration backstop: `pending-edit` → a genuine user edit outside the generated region, reconcile it (the rules below); `malformed` → duplicate/ambiguous markers, surface by name for a manual fix, never guess; `no-baseline` with `safeFirstWrite:false` → a pre-existing, never-reconciled file (surface it — decoration/hot-section will HOLD it rather than auto-write); `no-baseline` with `safeFirstWrite:true` → CORE created it this session, safe to establish its baseline; `missing`/`read-only` → surface plainly. `clean`/`generated-only` need no action. This is preflight/reporting only — it never resets a baseline; the writers each self-refuse at their own boundary regardless, so a skipped preflight degrades safely.
...
**Step 7 — Re-render PROJECT.md and update workspace meta.** Compose the six-section view (What & Why / State / People / Moves / Decisions & Risks / Notes) from the freshly-graduated units. Stamp the registry entry via the scripted writer — never by hand-editing `index.json`:
