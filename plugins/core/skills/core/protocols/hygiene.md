...
### Where the deeper sub-protocols live — and which one is retired

The edge-integrity sweep, archive reconciliation, file-cap reconciliation, and pattern synthesis have their long-form procedures in `references/hygiene-strategies.md`. Session-log auto-prune does not: it is retired without replacement (see the dream-cycle table below and `protocols/data-storage.md §"What's not in the v2 architecture"`) — `_sessions/` grows until the user prunes it, and no pass deletes a session log. An older reference that says the auto-prune "lives in hygiene-strategies" is historical.

There's no separate dream-cycle ritual — the operations got named for what they actually do, and they fire on the natural triggers (size, signals, edits) rather than on a fixed cadence. The context-window check (write the early summary stub under 30% remaining) lives in `protocols/startup.md`.
...
2. `_memories/INDEX-decisions.md` is auto-generated by `node "${CORE_ROOT}/skills/core/scripts/generate-unit-index.mjs" --kind decisions` (`CORE_ROOT` as resolved at startup; ships with the plugin by design) — walks `_memories/dc-*.md`, parses frontmatter, sorts chronologically. Invoke from the project root or pass an explicit memories dir; the script overwrites `INDEX-decisions.md` in place.
