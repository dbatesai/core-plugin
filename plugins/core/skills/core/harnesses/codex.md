...
**Exit-hook discharge DROPPED** on Codex; **startup catch-up is the equivalent** (same correctness, later timing — spec §9). On Claude Code a SessionEnd hook spawns a detached `claude -p "/finalize"` at session end, so the close runs itself. Codex has no validated SessionEnd hook that can spawn a surviving background process (see `hook-register` below), so there is no exit-hook close. Instead, every Codex startup runs the catch-up path (`startup.md §"Startup catch-up"`): `close-pass.mjs detect` finds an owed or partial close from last session and discharges the remainder before readiness. The per-op marker + single-flight lock + three-state detection are harness-agnostic — they work identically; only the *trigger* differs (Codex: next startup; Claude Code: session end). The user can still run `/finalize` manually any time. Re-open this drop if a live Codex install validates a SessionEnd-equivalent hook that can spawn a detached process.

**The same gap applies to `startup.md`'s decoration + index refresh backstop — OPEN, not fixed here.** That backstop's "runs unconditionally, every session" framing is only true once `startup.md` itself is actually being executed — on Claude Code, the `SessionStart` hook auto-invokes `/core`, which is what makes "every session" a real guarantee (see `hooks/hooks.json`). Codex has no `SessionStart`-equivalent hook (only `UserPromptSubmit` and `Stop` are wired — see `hooks/hooks-codex.json`), so nothing forces `startup.md` — and therefore the decoration backstop inside it — to run automatically at the start of a Codex session; it only runs when a returning Codex session actually loads `startup.md` (e.g., the user or `AGENTS.md` invokes `/core`/`configure-project` first), same structural limitation as the close-pass drop just above. This is NOT protection against the authorship-laundering bug itself — `decorate-graph.mjs`/`hot-section.mjs` refuse an unreconciled write in code regardless of harness or trigger — it only means the backstop's *proactive* value (catching a store that's gone stale between sessions before the user notices) is weaker on Codex than the "automatic every session" prose implies. A real fix would wire decoration into the earliest reliable Codex event (`UserPromptSubmit`) with a once-per-session guard (a persisted marker keyed by Codex's session/turn identity, checked and set on the first turn) so it doesn't re-run every single turn — that needs its own design pass (guard persistence, failure isolation so a decoration hiccup never blocks the user's turn, and hook-timeout budget headroom for a real store) and its own test coverage against the shipped, tested `retrieve-context-hook-codex.mjs`.
...
Codex has no startup-mandate hook the way Claude Code does, so a returning Codex session on a CORE folder confirms the install by running the `/configure-project` skill (or its script directly). It is the Codex-side counterpart to Claude Code's startup mandate: it resolves `CORE_ROOT`, confirms the manifests + skill dir + this adapter are present, runs the workspace fork-check in detect-only mode (a path already registered is a *returning* workspace regardless of which harness registered it — Codex opening a Claude-managed folder is not a fork), validates the `_memories/` store, reports configured-vs-session-live connector capability, runs the capability probe, and generates `AGENTS.md` from `CONTRACT.md` when one exists.
