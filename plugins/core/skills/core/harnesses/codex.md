...
CORE ships `hooks/hooks-codex.json` — registered via `.codex-plugin/plugin.json`'s explicit `"hooks"` field, deliberately not relying on the automatic-discovery default, because that default resolves to the same path as Claude Code's `hooks/hooks.json` and would risk Codex picking up Claude-only hook files. It wires exactly two doors: `UserPromptSubmit` → `retrieve-context-hook-codex.mjs` (per-turn retrieval) and `Stop` → `answer-close-hook-codex.mjs` (post-answer outcome close). Both are thin wrappers — pure JS, no shell env-var-prefix tricks, which don't work on Windows — that set `CORE_HOOK_HARNESS=codex` explicitly before delegating to the shared `retrieve-context-hook.mjs` / `answer-close-hook.mjs` implementations, which read Codex's `turn_id` as the answer-turn identity instead of Claude Code's `prompt_id`. Harness identity is never inferred from ambient env vars for hooks reached through these wrappers (`CODEX_SESSION_ID`/`CODEX_PLUGIN_ROOT` are undocumented, unreliable signals) — the wrapper sets it explicitly and authoritatively.

**The bound on that registration.** Two turn-scoped doors are all Codex gets: no session-start door, no session-end door, and no pre-execution block. Whether the two registered doors fire end to end is an installed-behavior question, answered per candidate by running the packaged plugin on a real Codex CLI and reading the receipts (`retrieve-context:delivered:ok` → `answer-close:closed:ok` with a native `turn_id`, joined into one outcome row). A proof at one commit does not carry to the next; re-run it against whatever candidate is current. Independent of that proof, the fallback inferred-closure path inside `retrieve-context-hook.mjs` covers a session where the `Stop` hook didn't fire for any reason, on either harness.
