...
**What is NOT yet true, named plainly:** this is verified against the documented contract and this repo's own fixture-based tests, not against a live Codex install. Nobody has installed this plugin on a real Codex session, trusted the bundled hooks (Codex requires explicit trust review before any plugin-bundled hook runs — "Codex skips them until the user reviews and trusts the current hook definition"), and confirmed two real turns produce a correctly joined retrieval/outcome row sharing one `turn_id`. That is Hale's or Crest's step, not buildable from a Claude Code session with no Codex CLI. Until that lands, the fallback inferred-closure path inside `retrieve-context-hook.mjs` remains a real, honest safety net — for a session where the Stop hook didn't fire for any reason, on either harness, not specifically because Codex "can't" support one.
