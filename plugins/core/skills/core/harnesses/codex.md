...
**Post-answer outcome close (Hale audit, 2026-07-17) DROPPED on Codex pending empirical validation of a Stop-equivalent.** Claude Code closes the per-turn retrieval hook's pending outcome via a real `Stop` hook (`hooks/answer-close-hook.mjs`, `harnesses/claude-code.md §hook-register`) firing once, right after the response completes, with the harness's own `prompt_id` as the answer-turn identity. Codex has no validated Stop-equivalent event, so on Codex the only mechanism is the fallback inferred-closure path inside `retrieve-context-hook.mjs`: it infers "the previous answer must be done" from the NEXT prompt arriving (sequencing, not post-answer observation) and generates a synthetic turn id rather than the harness's own. This is a named, honest limitation, not a silent gap — Hale's audit specifically flagged the old design for treating this inference as if it were real post-answer observation; it is real for Claude Code now, still inferred for Codex. Re-open once a live Codex install validates a Stop-equivalent lifecycle hook that fires once per completed turn.
