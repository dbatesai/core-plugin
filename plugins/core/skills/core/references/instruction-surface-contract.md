...
What it is: one canonical `<project>/CONTRACT.md` is the authoritative source of a project's agent instructions; the per-harness files — `CLAUDE.md` (Claude Code), `AGENTS.md` (Codex), `GEMINI.md` (Gemini) — are **generated** from it, not hand-maintained. This is the v3.0 maintenance-model change: you edit the contract, you regenerate; you don't edit three files in parallel and hope they stay in sync.
...
- `generate-claude-md.mjs` / `generate-agents-md.mjs` / `generate-gemini-md.mjs` — thin per-harness wrappers. Modes: `--mode write|check|dry-run`.
...
### gemini-only                ← emitted only to GEMINI.md
```

Canonical sections (everything except the Harness-Specific block) go to all three files; each `### <harness>-only` subsection is appended only to its harness's file.
...
1. **Migrate (draft):** `node migrate-to-contract.mjs --id <slug> --claude CLAUDE.md --codex AGENTS.md --gemini GEMINI.md --last-revised <date> --write CONTRACT.md`. Shared lines → canonical, unique → harness-only. The output is a **DRAFT** — the migrator refuses to overwrite an existing `CONTRACT.md` without `--force`, and it never auto-adopts.
