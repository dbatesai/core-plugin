...
**Step 1 — reason over exhaustive bounded shards.** The per-turn hook does the first part of this for you. When the keyword result is empty, or the prompt is a question (four or more content terms) whose ranking has no clear winner — the second-best body score is at least 0.8 of the best — the hook injects an escalation pack after the ordinary context: the first two candidate shards, up to 160 rows of `id — summary`, ordered by the write-time enrichment arm and then the substrate. Read the rows, pick the units that bear on what the question asks, Read those in full, cite them; if none apply, say so. The pack is capped at 32,768 bytes (`CORE_ESCALATION_BYTE_CAP` can only lower it) and truncated at a whole row. Prompts over 40 content terms (pasted reports, compaction continuations) never trigger, and a zero-hit on a one-word prompt or a slash command doesn't either. On a store where fewer than half the active units carry a current enrichment record, and two shards don't already cover everything, the pack is withheld (the shard order would be plain substrate order, which measured no lift) and the zero-hit directive stands in; the event says `escalation: unenriched`. `CORE_ESCALATION=0` turns the pack off and leaves the older text-only directive on a zero-hit; `CORE_ESCALATION_MIN_TERMS`, `CORE_ESCALATION_MAX_TERMS`, `CORE_ESCALATION_FLAT_FLOOR` and `CORE_ESCALATION_ENRICHMENT_FLOOR` move the trigger and the gate for harness sweeps. Replayed against 583 real prompts from this project's own transcripts the trigger fires on 16.5% (all question-shaped, none pasted); raising the flat floor to 0.9 drops that to 10.6% at the cost of 4 of 19 abstract gold questions. The retrieval event records `escalation: none | directive | shards | unenriched` and `shard_rows`, and `analyze-retrieval-quality.mjs` reports the escalation rate.

When the first two shards don't resolve it, or Tier 1 returned context that still doesn't answer the question, continue by hand from the next shard — a model-free hook cannot judge semantic sufficiency:

```bash
node "${CORE_ROOT}/skills/core/scripts/select-relevant-units.mjs" <project> "<the question>" --shard 0 --shard-size 80
```

The first line reports `Reasoning shard X/Y`, `units_scanned`, and `units_total`. Run every shard from `0` through `Y-1`; do not stop after the first plausible candidate. The order is recall-oriented: the shipped full-body product ranking comes first, followed by every unmatched active unit in deterministic id order, so the union of all shards covers the entire active corpus exactly once. Reason with world knowledge over each shard's id/topics/summary rows, Read the genuinely relevant units in full, then answer. This keeps query-time code model-free while using the already-active Claude/Codex model for the value-to-instance bridge (for example, "heritage" → El Primero). Log Tier 3 only after the reasoning pass actually runs; the hook's injected pack or directive alone remains an honest Tier 1 no-hit. The retrieval harness measures this path as the `escalated` arm: set `CORE_HARNESS_REASONER` to a command that reads the pack on stdin and prints `{"picks": [ids]}` (name its model in `CORE_HARNESS_REASONER_MODEL`), and the arm ranks the reasoner's picks ahead of the substrate on triggered queries; without one it prints `— (no reasoner configured)`.
