...
- **When to use:** at the end of a working session, so the next `/core` picks up clean. Skipping it is safe on Claude Code: the SessionEnd hook records a deterministic, zero-model lifecycle receipt for the session, and `/process-memory` back-fills the memory processing later. On Codex there is no SessionEnd hook and nothing records the close on its own — the owed close waits until the next session that starts with an explicit `/core`, whose catch-up step discharges it.
...
- **`/metrics export`:** the anonymized memory-metrics stats zip on your Desktop. **`/metrics self-test`:** author, verify, and score a blind test round now. The scheduled path authors nothing automatically — when the current round goes stale it stamps a trigger (at most once a week) and requests a fresh blind round, which is authored at your next user-invoked `/metrics` pass.
...
- **Capability & identity** — `resolve-plugin-root.mjs`, `capability-probe.mjs`, `capability-history.mjs`, `record-capability-snapshot.mjs`, `analyze-capability-drift.mjs`, `workspace-fork-check.mjs`, `project-slug.mjs`.
