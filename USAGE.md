...
- **`/metrics export`:** the anonymized memory-efficacy stats zip on your Desktop (what `/metrics-package` used to do). **`/metrics self-test`:** author, verify, and score a blind test round now (what `/self-test` used to do — the scheduled path also authors one automatically when the current round goes stale, at most once a week).
...
### `/metrics-package` *(deprecated shim — now `/metrics export`)*

Kept for one release so existing habits and scripts don't break; it redirects to `/metrics export` with a one-line notice. Removal scheduled for v3.15.0. Same exporter underneath: a zip on the Desktop with retrieval, recognition, store-health, hygiene, and capability statistics — numbers, dates, fixed vocabulary, and salted pseudonyms only; real project content never enters it by construction.
