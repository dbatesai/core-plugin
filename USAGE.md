...
Run the blind, round-based memory self-test on this project's own store — the honest counterpart to `/metrics`: not "does the machinery round-trip" but "can the memory actually be found from natural questions, and does the system correctly stay quiet when the answer genuinely isn't there?"

- **What it does:** freezes the store's identity for a round, emits a blind-authoring brief, then a separate agent authors a fresh question set reading only the stored facts (never the retrieval code) — including questions the store *should* answer with no shared wording, questions about change over time, and questions the store deliberately *cannot* answer (where the right behavior is "nothing stored about that", plus false-premise questions that swap in something the store never mentions). The set is mechanically verified (schema, zero word-overlap checked against title/body/topics, the false-premise entity checks, per-kind counts, corpus identity) and **frozen** before anything is scored. Then the real retrieval path is measured against it, with a per-kind breakdown, the unanswerable trap-leak rate, and an **old-vs-new-round delta** that watches for the store being tuned to its own test. Rounds are append-only; a new round is the only way to add questions.
- **When to use:** "test the memory on this project," "author a fresh question set and score it," "run a blind retrieval self-test," or whenever you want a self-generated, honestly-labeled answer key rather than the small static one. Once a round exists, `/metrics` prefers it automatically.
- **Writes:** an append-only round directory under `<project>/_tests/self-test/round-<N>/` (brief, frozen corpus identity, the verified question set, the pre-registration record, and one results record per run). Reads are read-only to the store.
...
- **Self-measurement** — `metrics-init.mjs`, `classify-turns.mjs`, `metrics-rollup.mjs`, `metrics-detectors.mjs`, `calibrate-classifier.mjs`, `metrics-check.mjs` (the `/metrics` live evidence check: round-trip probe + store health + calibration-pool progress; prefers a frozen self-test round when one exists), `self-test-round.mjs` (the `/self-test` round manager: freezes the corpus, emits the blind-authoring brief, mechanically verifies + freezes an authored question set, runs the real harness against it, computes the old-vs-new-round overfitting delta), `log-event.mjs`, `record-retrieval-event.mjs`, `analyze-retrieval-quality.mjs`, `analyze-retrieval-skip.mjs`, `read-transcript.mjs`.
